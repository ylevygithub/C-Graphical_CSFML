...
{
	char *str; //on declare la string str.

	if (ac != 2) //si le nombre d arguments est different de 2, le programe se termine.
		return (0);
	str = av[1]; //str prend comme valeur le 2em argument.
	if (str[0] == '-' && str[1] == 'h') { //si on a ./my_runner -h on affiche la suite sur le terminal.
		write(1, "USAGE\n\t./my_runner map.txt\n\n", 28);
		write(1, "OPTIONS\n", 8);
