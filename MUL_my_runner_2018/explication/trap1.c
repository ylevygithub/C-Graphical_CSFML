...
void play_again(window_t *win)//le jeu recommence
{
	win->data->jump = 0;
...
void game_stop(window_t *win)//le jeu recommence avec une vie en moins
{
	win->data->end = 1;
	win->data->start = 0;
	win->data->life = 3;
...
void set_frame_blood(window_t *win)//mise en place de la texture du sang lorsque le player meurt
{
...
void set_death(window_t *win)//si le player touche une trape, il pert une vie jusqua ne plus en avoir assez et c game over.
{
	sfVector2f pos;
