...
void destroy_win(window_t *win)
{
	...
	sfTexture_destroy(win->plx->one->tex);
	sfSprite_destroy(win->plx->one->spr);
	sfTexture_destroy(win->plx->two->tex);
	sfSprite_destroy(win->plx->two->spr);
	sfTexture_destroy(win->plx->three->tex);
	sfSprite_destroy(win->plx->three->spr);
